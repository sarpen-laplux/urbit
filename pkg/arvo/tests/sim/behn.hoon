...
++  test-behn-doze-replace
  ...
  =/  r2  (take:~(. sim-behn sim.r1) t0 /behn [%doze `(add t0 ~m5)])
  (expect-eq !>([[%timer-stop 1] [%timer-start 1 ~m5] ~]) !>(fx.r2))
::  +test-behn-doze-cancel: %doze ~ just stops
::
++  test-behn-doze-cancel
  =/  sim  make
  =/  r1  (take:~(. sim-behn sim) t0 /behn [%doze `(add t0 ~m1)])
  =/  r2  (take:~(. sim-behn sim.r1) t0 /behn [%doze ~])
  ;:  weld
    (expect-eq !>([%timer-stop 1]~) !>(fx.r2))
    (expect-eq !>(|) !>(alm.sim.r2))
  ==
...
++  test-behn-past-doze
  ...
  (expect-eq !>([%timer-start 1 `@dr`0]~) !>(fx.r))
::  +test-behn-fire: timer fire plans %wake and arms the backstop
::
++  test-behn-fire
  =/  sim  make
  =/  r1  (take:~(. sim-behn sim) t0 /behn [%doze `(add t0 ~m1)])
  =/  r2  (call:~(. sim-behn sim.r1) (add t0 ~m1) [%timer-fire 1])
  ;:  weld
    (expect-eq !>([%timer-start 1 ~m10]~) !>(fx.r2))
    (expect-eq !>([[/behn [%wake ~]] ~]) !>(ova.r2))
    (expect-eq !>(&) !>(alm.sim.r2))
  ==
...
++  test-behn-wake-retry
  ...
  =/  r2  (call:~(. sim-behn sim.r1) t0 [%news /behn %bail %soft])
  =/  r3  (call:~(. sim-behn sim.r2) t0 [%news /behn %bail %soft])
  ;:  weld
    (expect-eq !>([[/behn [%wake ~]] ~]) !>(ova.r1))
    (expect-eq !>([[/behn [%wake ~]] ~]) !>(ova.r2))
    (expect-eq !>(~) !>(ova.r3))
    %+  expect-eq
