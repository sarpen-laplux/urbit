  ...
  ++  request
    ...
    ?-    -.action
      ...
      =/  res=toon
        ...
        %+  slam
          %+  slam  gat
          !>([[now=now tick=tick eny=eny bek=bek] ~ ~])
...
::
++  scry-mime
  |=  [now=@da tick=@ud rof=roof =gang ext=(unit @ta) pax=path]
  |^  ^-  (each mime tape)
  ::  parse
  ::
...
++  scry
  ...
  ?:  &(?=(%x ren) ?=([%range @ @ @ *] tyl))
    ...
    =/  mym  (scry-mime now tick rof lyc p.pork [%$ vew (en-beam -.bem q.pork)])
    ?:  ?=(%| -.mym)  ~
    =*  mime  p.mym
  ...
  ?:  &(?=(%x ren) ?=(%$ syd))
    ...
    ::
        [%'_~_' *]
      =/  mym  (scry-mime now tick rof lyc (deft:de-purl:html tyl))
      ?:  ?=(%| -.mym)  [~ ~]
      ``noun+!>(p.mym)
    ==
  ?.  ?=(%$ ren)  ~
